...
Fixes:
  ...
  FIX D: T3 starting diameter uses D_sc_outer for solid Al/Pb sheath
         ...
         T3 buried: x 1.6 for touching trefoil
         T3 duct:   x 1.0 (cables not touching inside duct)
         T3 air:    x 1.0 always (TB880 confirmed)
"""
...
def thermal_resistances(c, r, log):
    """Populate r with T1, T2, T3_buried, T3_duct, T3_air, T4_buried."""
    ...
    # ── T3 — oversheath ───────────────────────────────────────────────────
    # Starting diameter for T3:
    # Solid metallic sheath (Al, Pb): D_start = ds + ts  (outer surface)
    #   TB880 CS0: D'a = 67.7 + 0.8 = 68.5mm → T3 = 0.05420 K.m/W  ✓
    # Cu wire screen + foil: D_start = D_fl
    # Wire/tape screen (no foil): D_start = ds (mean)
    ...
    # Buried: 1.6x for touching trefoil/flat
    Tf_buried = touching_factor(c['formation'], c['spacing_s'], c['De'],
                                c['t3_touching_factor'])
    T3_buried = Tf_buried * T3_raw

    # Duct: NO 1.6x — cables are spaced inside the duct, not touching
    T3_duct   = 1.0 * T3_raw

    # Air: NO 1.6x — TB880 CS0-4 confirmed
    T3_air    = 1.0 * T3_raw

    r['T3_buried']        = T3_buried
    r['T3_duct']          = T3_duct      # ← KEY: used by iterate_duct
    r['T3_air']           = T3_air
    r['T3_raw']           = T3_raw
    ...
    log.append(('Thermal Resistances  [IEC 60287-2-1]', [
        ...
        ('T3 duct  x1.0 (cables not touching in duct)',
         f"{T3_duct:.10f}", 'K.m/W'),
        ...
        ('T4 duct    (iterative theta_m — see duct section)',
         'see duct section', ''),
        ('T4 air     (iterative x — see air section)',
         'see air section', ''),
    ]))